...
> **Setup:** Ensure the Kite MCP server is configured in `~/.claude/mcp.json` or your project's `.mcp.json` with the server name `kite`. If you used a different name, replace `kite` with your server name throughout this skill. See the [setup guide](../README.md) for details.
